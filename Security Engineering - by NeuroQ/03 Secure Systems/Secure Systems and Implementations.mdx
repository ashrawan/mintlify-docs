...
This document provides a concise, research-grade overview of critical security engineering domains, focusing on architectural principles, implementation strategies, and governance for secure systems. It synthesizes fundamental web application security, robust authentication and authorization mechanisms, advanced access control, federated identity, privileged access management, identity governance, and foundational directory services.
...
```mermaid
graph TD
    subgraph "OAuth 2.0 Grant Types"
        A[Authorization Code Grant] --> A1[User-Agent Redirect]
        A1 --> A2[Authorization Code]
        A2 --> A3[Token Exchange with PKCE]
        A3 --> A4[Access Token + Refresh Token]

        B[Refresh Token Grant] --> B1[Refresh Token]
        B1 --> B2[New Access Token]

        C[Implicit Grant - DEPRECATED] --> C1[Direct Access Token in URL]

        D[Client Credentials Grant] --> D1[Client Authenticates]
        D1 --> D2[Access Token for Client Resources]
    end

    subgraph "OAuth 2.0 Authorization Code Flow"
        User[User] -->|1. Request protected data| Client[Client Application]
        Client -->|2. Redirect with Auth Code Request + PKCE challenge| AuthZ[Authorization Server]
        AuthZ -->|3. Prompts for login/consent| User
        User -->|4. Authenticates & grants consent| AuthZ
        AuthZ -->|5. Redirect with Authorization Code| Client
        Client -->|6. Exchanges Code & PKCE verifier for Tokens| AuthZ
        AuthZ -->|7. Issues Access Token & ID Token| Client
        Client -->|8. Presents Access Token for API access| Resource[Resource Server]
        Resource -->|9. Returns protected resource| Client
    end
```
